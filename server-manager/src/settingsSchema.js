...
const serverSettings = [
  ...
  { key: 'playersInheritBaseSpells', label: 'Players inherit base spells', type: 'bool', group: 'Gameplay', help: 'On (default): player characters carry the Player record\'s castable spells (Flames, Healing). Off: characters start without them; abilities and racial passives stay. Read by the native server at boot.' },
  ...
  { key: 'logDir',         label: 'Log directory',   type: 'text',   group: 'Data & storage', placeholder: 'C:\\logs', help: 'Where chat.log and service logs are written. Overridden by the DRAGONBREAK_LOG_DIR env var.' },
...
const backendEnv = [
  ...
  { key: 'NAME_TABLE_PATH',        label: 'Name table',            type: 'text',   group: 'Website profiles', help: 'name-table.json the game server writes at start, for race and place names on the profile page. Empty = build/dist/server/name-table.json.' },
  ...
  // CI & tooling
  { key: 'DRAGONBREAK_GH_TOKEN',     label: 'GitHub token (PAT)',    type: 'secret', group: 'CI & tooling', help: 'PAT with actions:write - powers the manager CI Rebuild (workflow dispatch).' },
]

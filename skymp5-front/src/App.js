      ...
      return (
        <div style={{ position: 'static' }}>
          {this.state.widgets.map((widget, index) =>
            <Constructor
              key={(widget.type === 'trade') ? ('trade-' + widget.id) : (widget.type === 'adminPanel') ? ('adminPanel-' + widget.id) : (widget.type === 'contextMenu') ? ('contextMenu-' + widget.id) : (widget.type === 'emoteWheel') ? ('emoteWheel-' + widget.id) : (widget.type === 'housing') ? ('housing-' + widget.id) : (widget.type === 'mastery') ? ('mastery-' + widget.id) : (widget.type === 'bountyBoard') ? ('bountyBoard-' + widget.id) : (widget.type === 'interactPrompt') ? ('interactPrompt-' + widget.id) : (widget.type === 'hud') ? ('hud-' + widget.id) : (widget.type === 'reading') ? ('reading-' + widget.id) : (widget.type === 'labour') ? ('labour-' + widget.id) : (widget.type === 'lockpick') ? ('lockpick-' + widget.id) : (widget.type === 'pigeon') ? ('pigeon-' + widget.id) : (widget.type === 'faction') ? ('faction-' + widget.id) : (widget.type === 'mailMarkers') ? ('mailMarkers-' + widget.id) : (widget.type === 'rite') ? ('rite-' + widget.id) : (widget.type === 'skinning') ? ('skinning-' + widget.id) : (widget.type === 'characterSelect') ? ('characterSelect-' + widget.id) : (widget.type === 'dungeonGate') ? ('dungeonGate-' + widget.id) : (widget.type === 'party') ? ('party-' + widget.id) : (widget.type === 'tomeShop') ? ('tomeShop-' + widget.id) :(widget.type === 'charCreator') ? 'charCreator' : (index.toString() + widget.type + ((widget.type === 'form') ? widget.elements + widget.caption : 'chat'))}
              dynamicSize={true}
              elem={widget}

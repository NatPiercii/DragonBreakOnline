...
//
// A jail zone is an interior cell: every jail and prison in the load order (jail-cells.json, generated by
// ck-mcp/jails.py from the crime factions' jail markers and Jail/Prison cell names), gamemode-config "jail.cells",
// and any an admin adds in game with /jail add (kept in jails.json, runtime data, never tracked). /unstuck is
// refused inside one, and anywhere while a sentence is unserved.
//
...
module.exports = (api) => {
  ...
  let added = readAdded();
  const GENERATED = (() => { try { const v = JSON.parse(fs.readFileSync(path.resolve('jail-cells.json'), 'utf8')); return Array.isArray(v.cells) ? v.cells.map(String) : []; } catch (e) { log('jail: jail-cells.json unreadable', e.message); return []; } })();
  const rawCells = () => GENERATED.concat((CFG.cells || []).map(String), added);
  let jailSet = new Set(rawCells().map(norm));
  const jailCells = () => jailSet;
  const isJail = (cell) => !!cell && jailSet.has(cell);
  const jailName = (cell) => {
    try {
  ...
  registerChatCommand('jail', (a, args) => {
    ...
    if (sub === 'list') {
      ...
      if (isJail(here)) return personal(a, `${list.length} jail zones. You are in one: ${jailName(here)}.`);
      return personal(a, `${list.length} jail zones. Added in game: ${added.length ? added.map((c) => jailName(norm(c))).join(', ') : 'none'}.`);
    }
    ...
    if (sub === 'add') {
      ...
      added = added.concat([hereRaw]); saveAdded(added); jailSet = new Set(rawCells().map(norm));
    ...
    if (sub === 'remove') {
      if (!added.some((c) => norm(c) === here)) return personal(a, isJail(here) ? 'That jail comes from the load order or gamemode-config and stays.' : 'This cell is not a jail zone.');
      added = added.filter((c) => norm(c) !== here); saveAdded(added); jailSet = new Set(rawCells().map(norm));
